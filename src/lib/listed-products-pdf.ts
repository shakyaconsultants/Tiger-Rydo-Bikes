...
async function addBrochurePages(doc: JsPdfDoc, product: ListedProduct) {
  ...
  doc.setFontSize(11);
  doc.text(formatPrice(product.price), pageWidth / 2, pageHeight - 28, { align: "center" });
  doc.setFontSize(9);
  doc.text("Tiger Rydo Product Brochure", pageWidth / 2, pageHeight - 18, { align: "center" });
  ...
  leftY = drawSpecRow(
    doc,
    "Tyre Size",
    [brochure.tyreSizeFront && `Front ${brochure.tyreSizeFront}`, brochure.tyreSizeRear && `Rear ${brochure.tyreSizeRear}`]
      .filter(Boolean)
      .join(", "),
    leftX,
    leftY
...
async function addCatalogTable(doc: JsPdfDoc, products: ListedProduct[]) {
  ...
  doc.setFontSize(9);
  doc.text(`Generated: ${new Date().toLocaleDateString("en-IN")}`, pageWidth - 14, 14, {
    align: "right",
  });
  ...
  const productsWithImages = await Promise.all(
    products.map(async (product) => ({
      ...product,
      image: product.imageUrl ? await getBase64Image(product.imageUrl) : null,
    }))
  );
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      ...
      1: { cellWidth: 45, minCellHeight: 20, halign: "center" },
    ...
    didDrawCell: (data) => {
      if (data.section === "body" && data.column.index === 1) {
        ...
        try {
          ...
          doc.addImage(img, imageFormat(img), x, y, size, size);
        } catch {
          // ignore
        }
      }
...
export async function downloadListedProductsPdf(products: ListedProduct[]) {
  ...
  const catalogOnly = products.filter((p) => !normalizeBrochure(p.brochure).enabled);
  ...
  if (catalogOnly.length > 0 || brochureProducts.length === 0) {
    if (started) doc.addPage();
    await addCatalogTable(doc, catalogOnly.length > 0 ? catalogOnly : products);
  }

  doc.save("tiger-rydo-product-brochure.pdf");
}
